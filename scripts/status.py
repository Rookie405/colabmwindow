...
key = open("/content/api-key.txt").read().strip() if os.path.exists("/content/api-key.txt") else ""
models = sh("curl -s -m 5 -H 'Authorization: Bearer %s' http://127.0.0.1:%s/v1/models" % (key, PORT)).strip()
running = bool(sh("pgrep -f 'api_server.py|tabby|exllamav3|llama-server'").strip())
